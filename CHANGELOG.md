...
Update Manager now fetches GitHub's own release notes directly whenever an entity has nothing useful to show, which happens a lot more often than you'd expect. Skipping several versions at once now shows what changed in all of them, not just the newest, and a downgrade shows the same thing in reverse: every version you're stepping back through, oldest to newest, ending on the one you just left, with the release-announcement link corrected to match instead of always pointing at the newest release.

Some add-ons even showed their entire, years-long changelog instead of just what changed. That's fixed, and add-ons that just wrap another project (like Zigbee2MQTT, Mealie, or Matterbridge) now also show that project's own, more detailed release notes alongside their own short note.

**Also in this release:**
- The refresh button now genuinely checks for updates too, skipping anything already installing.
- History shows whether a backup was requested before an auto-install.
- The pending-update dialog shows when an update was (or will be) announced, and now refreshes itself automatically when a newer version is found in the background.
- Opening an update that's already installing now shows Home Assistant's own dialog, with the real progress bar, instead of ours.

### Added
- Both the pending-update dialog and History now fall back to fetching GitHub's own release notes directly whenever an entity's own sources have nothing to show -- confirmed live this is common, not a rare edge case: HACS's own `async_release_notes()` returns `None` whenever `pending_restart` is still true or the installed version isn't in its own `published_tags`, and Home Assistant Supervisor's own update entity doesn't support release notes at all. When skipping several versions at once, the notes now cover every skipped release, not just the newest one, the same gap HACS's own update entity already closes for itself when it can -- betas/RCs GitHub happened to tag along the way are left out of that compilation (confirmed live against Home Assistant OS: jumping 18.1 -> 18.2 no longer drags in 18.2.rc1's own changelog), unless the jump itself is genuinely to a beta/RC. A downgrade shows the full range too, just oldest first and explicitly including the version you moved away from (unlike an upgrade, which never shows the version you already had) -- direct user feedback from two real downgrades (7.1.10 -> 7.1.9, 2.0 -> 1.0): an entity's own `release_url` always reflects "latest available", never "what you actually landed on" (confirmed against HACS's own source), so both the notes and the "Open release announcement" link are matched against the version actually being described (`u.latest_version` for a pending update, `entry.to_version` for History) and corrected before anything is fetched, rather than trusting `release_url`'s own embedded tag blindly. Uses the linked GitHub account's own token for a higher rate limit when available, but doesn't require linking one. Also converts GitHub's own `:shortcode:` emoji syntax (confirmed live in home-assistant/supervisor's real release notes -- its own release-drafter config headers every section that way) into the real emoji glyph, and turns `#1234` issue/PR references and `@username` mentions (including GitHub's own auto-generated "bumped by @dependabot[bot] in #1234" format, extremely common in practice) into real links, since `ha-markdown` doesn't understand any of those and would otherwise show the literal text -- a bot's own link goes to `github.com/apps/{name}`, not `github.com/{name}` (which 404s for most bots).
- **Release notes now also surface the real upstream project's own notes for add-ons that just wrap an external app** -- direct user feedback, three real examples: a Zigbee2MQTT add-on's own release notes link straight to Koenkk/zigbee2mqtt's exact release, a Mealie add-on's own release notes link to mealie-recipes/mealie's general releases page with no specific tag at all, and a Matterbridge add-on's own changelog linking a sub-dependency via a plain `github.com/OWNER/REPO/blob/.../CHANGELOG.md` link rather than a `/releases/...` one. All are common shapes for add-ons that are thin wrappers: their own release notes are often just a one-line "bumped to X" note, while the real, detailed explanation lives upstream. Shown as a clearly separate, labeled "OWNER/REPO's own release notes:" block underneath the add-on's own notes, never replacing them. Only acts when exactly one distinct external repo is referenced, scoped to just the version range actually being shown, not an entity's entire, possibly years-long release-notes text (several dependency bumps mentioned in the same range is deliberately left alone, guessing which one is "the" upstream project among several isn't reliable); when the embedded link already names a specific release, that's used directly, otherwise the version already being described is tried as the tag (confirmed live: an add-on's own version number frequently *is* the upstream tag verbatim), and a `.../releases/latest` link (a real, common GitHub URL shape that just redirects to whatever's newest) is recognized as no specific tag rather than a literal tag named "latest". A link to a non-GitHub domain (Matterbridge's own real changelog links its *main* upstream project to a custom-domain mirror, `matterbridge.io`, confirmed to be the exact same content as its real GitHub changelog, just HTML-rendered) can't be resolved, but still counts toward "how many distinct references are there", so an entry mentioning both kinds never risks picking the resolvable one and quietly showing the wrong project's notes. Silently shows nothing extra whenever no link is found, several distinct ones are, or the guessed tag doesn't exist.
- The pending-update dialog now shows when an update was (or, for a still-"waiting" one, would be) announced, right next to Current/New version -- direct user feedback: "bij postponed zie je niet wanneer hij announced gaat worden en bij ready net zo goed", matching a fact History already showed for a completed install but the live dialog didn't show anywhere. A "ready" update with an active auto-install countdown shows the real announcement time under "Announced"; a "waiting" one shows a projected one under "Announcement" instead -- deliberately a different label, since "Announced" would assert something that hasn't actually happened yet.
- History now shows whether a backup was actually requested before an auto-installed update, right next to "Install method" -- direct user feedback: "we tonen nergens of die backup ook echt gelukt is". Only ever shown for auto-installs, never a manual one: install_manager.py's own auto-install dispatch unconditionally requests a backup whenever the entity supports it, so that's a sure fact, but a manual install could just as easily have gone through HA's own native dialog and its own separate backup checkbox, which we can't see at all -- rather than guess, that row is simply left out for a manual install.
- The panel's own manual refresh button now also does a real "check for updates", the same `homeassistant.update_entity` service HA's own Settings > Updates page calls for that action: every `update.*` entity on the system is told to actually re-poll its integration for a brand-new version right now, not just refresh whatever this integration's own coordinator already had cached -- direct user feedback ("kan die knop ook 'check for updates' doen? zoals ha zelf doet"). Skips anything already installing: forcing a fresh poll on a busy entity risked its integration reporting a momentarily inconsistent state, which this integration's own tracking could misread as "no longer pending" and drop entirely, taking the row (and its spinner) with it -- there's no real reason to check for an even newer version on something already mid-install anyway.

### Changed
- **Our own dialog is never shown while an update is actually installing anymore -- HA's own more-info dialog opens directly instead, with the real, live progress bar** -- direct user feedback: "als hij installing is wil ik niet onze dialog tonen maar direct die van ha waarin de progress zichtbaar is". Removes the green "Installing…" banner our own dialog used to show (a status color left over from whatever this entity's status was a moment before, not actually meaningful once installing) both when opening the dialog while already installing, and when an install starts while it's already open (however that install got triggered: our own dialog, "Update all", auto-install, or the rollout queue).
- **The Release notes section now shows a small loading placeholder instead of just appearing suddenly once its fetch resolves** -- confirmed against Home Assistant core's own native update dialog (`more-info-update.ts`), which solves the exact same "content pops in and shifts the dialog" problem this same way, not with a fixed delay. Reserves the same small, stable amount of space the whole time notes are still loading, so removing it never itself causes a jump either. The separate fixed delay before the dialog becomes visible at all is bumped from 100ms to 150ms, since Release notes specifically no longer needs to lean on that alone.
- **The release-announcement link now always lives inside the "Release notes" section, in both the pending-update dialog and History, instead of sitting separately in the facts block above it** -- direct user feedback: "als die notes ontbreken, dan staat die link bij de details ipv onder het kopje 'Release notes'". When notes are shown, the link appears right after them; when none could be found anywhere, the section still appears with just the link, so it's never orphaned outside "Release notes" again. Also reworded from "Read release announcement" to "Open release announcement" -- "Read" implied you hadn't seen it yet, which read oddly directly under notes you're already looking at.
- The pending-update dialog's Release notes and Community sections now have their own heading, same as History already did -- direct user feedback: History being the only labeled section read as inconsistent once pointed out. Applied inside each individual History entry's own expanded facts too, for the same reason ("dit consistent doorvoeren binnen een history item"): its own changelog/release-notes block and its per-entry Community section (they share the same code, so it got the heading for free) now both have one as well.
- History's own GitHub-notes fallback now only fetches once an entry is actually expanded, not for every entry the moment the History dialog opens -- found during a `/simplify` cleanup pass: it was firing unconditionally for every entry lacking its own release notes, the exact N-live-requests-per-open problem the neighboring Community section was already carefully built to avoid. The manual refresh button's own "check for updates" wait and its community-verdict refresh now also run alongside each other instead of one after the other, since neither depends on the other's result -- shortens the wait before the "refreshed" toast.
- The panel's own manual refresh button now also force-checks every locally remembered vote against live community-votes data, forgetting any that are genuinely gone even within the usual 5-minute grace period (`_STALE_VOTE_GRACE_PERIOD`, see 0.5.1's own "you voted" fix below) -- direct user feedback: clicking refresh felt like the obvious way to skip that wait, and now it is. The passive per-dialog check still respects the grace period as before; only this explicit, deliberate action bypasses it.
- The Updates list's own trailing timer pill ("Today 21:30", "Tomorrow 09:00") now drops the "Today"/"Tomorrow" word whenever the bare clock time alone already makes it unambiguous -- direct user feedback: that pill sits right next to the entity's own title on the same row, and the extra word was sometimes wide enough to crowd the title out. The dialog's own status sentence and History's detail rows are unaffected, both still show the full word since they have the room and genuinely benefit from it.

### Fixed
- **A downgrade could show a HACS entity's own frozen History release notes for a completely different, newer version than the one actually installed** -- direct user feedback, a real 7.1.10 -> 7.1.9 downgrade, "hier verwacht ik ook de notes van 7.1.9 te zien ipv 7.1.10". HACS's own `async_release_notes()` (its own docstring literally says "Compile release notes from installed version up to the latest") has no concept of a downgrade at all, so it compiles everything from whatever it still considers "latest" down to the downgrade target, without that target's own entry ever actually being included. History now recognizes this specific shape (the version you moved *from* has its own heading in the frozen text, but the version you actually landed on doesn't) and falls back to fetching the correct release directly instead of trusting the frozen, wrong-for-this-jump snapshot.
- **Some add-ons showed their entire, years-long changelog as "release notes" instead of just the relevant version(s)** -- direct user feedback, two real examples (matterbridge-home-assistant-addon, alexbelgium's mealie add-on). Root cause traced to HA core itself: `SupervisorAddonUpdateEntity.async_release_notes()` tries to slice its own changelog file down to the relevant range with a regex that requires a version heading to have *nothing* else on the same line, so a heading like `## 2026.7.5 - 2026-07-31` or `## v3.22.0 (2026-08-01)` (both extremely common shapes, trailing date or a "v" prefix) fails to match and silently falls back to the whole, unfiltered file. Every release-notes source now gets an additional, looser pass applied to it (a version just needs to appear anywhere in a heading line, not be the entire remainder) before being shown -- a safe no-op for anything that was already correctly scoped, and a real fix (14000+ characters down to a couple hundred, confirmed against both real changelogs) for anything HA's own regex silently failed on. That looser matching is careful not to match a version string inside a longer, unrelated one sharing the same prefix (confirmed against a real one: alexbelgium's mealie changelog has both "v3.17.0-1" and, separately, "v3.17.0" as their own entries) by requiring the version isn't immediately continued by more version-like characters.
- **Release notes were re-fetched from scratch every time the same dialog was reopened, even though nothing about that release could have changed** -- direct user feedback: "als ik een dialog vaker achter elkaar open moet hij die content telkens opnieuw ophalen. overbodig toch?" Both the GitHub-notes fallback and HA's own native release-notes call are now cached in-memory for the panel's lifetime, keyed by the exact version(s) involved so a newly-available version never serves another version's stale cached notes. The community verdict is deliberately left uncached -- votes genuinely can change between two opens, unlike a release's own notes.
- **A pending-update dialog left open didn't refresh when a newer version was discovered in the background** -- direct user feedback: "als er op de achtergrond ontdekt wordt dat er een nieuwe update beschikbaar voor is, dan ververst de dialog niet. Dan moet ik 'm eerst sluiten en nog eens openen." The panel already noticed when an install actually finished (installed_version changing) and refreshed accordingly, but never noticed latest_version changing on its own (a newer version becoming available with nothing being installed), so an open dialog just kept showing the stale "New version" fact indefinitely. Now treated the same way: the dialog quietly refreshes itself in place.
- **A detail dialog visibly shifted right after opening**, once the Community section and/or Release notes finished their still-in-flight fetch and popped into place -- direct user feedback: "de dialogs lijken allemaal vlak na het openen te verspringen omdat er dan nieuwe data is geladen". A short, fixed 100ms delay before the dialog actually becomes visible now lets most of these land first; short enough to still feel instant, not a guarantee for a genuinely slow fetch (an explicit, accepted trade-off over the more correct but bigger "wait for the actual fetches" fix).
- **A pending update's release notes could render below its History section instead of above it**, direct user feedback (seen live on a browser_mod pending update). Both sections are appended synchronously in the right order in the code, but release notes are only ever inserted once their async fetch resolves -- whenever that took a moment longer than History's own (fully synchronous) append, History ended up landing first regardless of code order. Fixed with a fixed anchor point release notes always insert themselves before, independent of how long the fetch takes.
- **A History item's facts (Available since/Announced/Installed/etc.), its "Read release announcement" link, and its release notes all rendered noticeably smaller than the exact same content in the pending-update dialog above it** -- direct user feedback: "binnen history gebruiken we andere fontsizes voor de release notes", then a follow-up check caught the release-announcement link too. All three had simply been missed when the History card's own compact 13px sizing was first introduced (its per-entry Community section and summary line already had this same exception carved out, with documented reasoning; these never did).
- **A History item's spacing around its dividers and headings was fixed piecemeal, one hand-placed margin at a time, and kept turning up a new inconsistency each time** -- direct user feedback: "de witruimte rondom dividers en kopjes is nog steeds inconsistent. fix dat. grondig." Replaced every one-off margin-top with a single flex gap on the actual shared parent (`.dialog-history-notes-wrap`, previously not a flex container at all) plus a smaller, History-scoped gap for the individual fact rows -- the facts block, Community section, release-notes divider, its heading, and its content now all get uniform, consistent spacing from one rule instead of five.
- **History's own "No updates logged yet" empty-state card rendered at the full page width instead of the same 600px column every other card on the page uses** -- direct user feedback: "card valt buiten het grid". The exact same class of bug as an earlier, already-fixed one (the Updates tab's own empty state and rollout-queue cards): this card is appended directly to the page grid itself, a sibling level the shared width-capping selector list didn't cover, so it silently fell through with no width cap or centering at all. Added to that same selector list rather than given its own separate rule.
