...
class TestEvaluateStagingDefaults:
    ...
    def test_small_ready_immediately_with_default_rules(self):
        result = staging.evaluate_staging("small", available_since=NOW, now=NOW)
        assert result.status == "ready"
        assert result.remaining is None

    def test_medium_waiting_right_after_appearing(self):
        result = staging.evaluate_staging("medium", available_since=NOW, now=NOW)
        assert result.status == "waiting"
        assert result.remaining == timedelta(days=7)

    def test_medium_still_waiting_partway_through(self):
        now = NOW + timedelta(days=3)
        result = staging.evaluate_staging("medium", available_since=NOW, now=now)
        assert result.status == "waiting"
        assert result.remaining == timedelta(days=4)

    def test_medium_ready_exactly_at_the_wait_boundary(self):
        now = NOW + timedelta(days=7)
        result = staging.evaluate_staging("medium", available_since=NOW, now=now)
        assert result.status == "ready"
        assert result.remaining is None

    def test_medium_ready_well_past_the_wait(self):
...
class TestEvaluateStagingCustomRules:
    def test_custom_rules_override_small_and_medium(self):
    ...
    def test_zero_wait_custom_rule_for_medium(self):
        rules = staging.StagingRules(
